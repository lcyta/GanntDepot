...
def manejar_feriado_individual(nombre, feriados):
    with st.expander("➕ Agregar / ❌ Eliminar feriado individual"):
        nueva_fecha = st.date_input("Fecha del feriado", value=date.today(), key=f"nueva_fecha_{nombre}")
        ...
        seleccionado = st.selectbox("Seleccioná feriado a eliminar", opciones, key=f"elim_{nombre}")
