...
def get_shipment_path(project_name):
    ...
    return os.path.join(base_path, f"{project_name}_shipment.json")

def load_shipment(project_name):
    path = get_shipment_path(project_name)
    if not os.path.exists(path):
        return []
    with open(path, "r", encoding="utf-8") as f:
        return json.load(f)

def save_shipment(project_name, accesorios):
    path = get_shipment_path(project_name)
    with open(path, "w", encoding="utf-8") as f:
        json.dump(accesorios, f, ensure_ascii=False, indent=2)

def add_shipment(project_name, nuevo_accesorio):
    accesorios = load_shipment(project_name)
    accesorios.append(nuevo_accesorio)
    save_shipment(project_name, accesorios)

def delete_shipment(project_name, accesorio_nombre):
    accesorios = load_shipment(project_name)
    accesorios = [a for a in accesorios if a["Nombre"] != accesorio_nombre]
    save_shipment(project_name, accesorios)

def update_shipment(project_name, nombre_original, accesorio_actualizado):
    """
